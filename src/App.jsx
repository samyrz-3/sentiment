...
import { fetchMarket, analyzeSegment, checkHealth } from "./api.js";

const SIGNAL_CFG = {
  BEAT:     { color: "#22e5a0", icon: "▲", bg: "rgba(34,229,160,0.07)"  },
  MISS:     { color: "#ff3d5a", icon: "▼", bg: "rgba(255,61,90,0.07)"   },
  INLINE:   { color: "#f5c842", icon: "≈", bg: "rgba(245,200,66,0.07)"  },
  WARNING:  { color: "#f5c842", icon: "⚠", bg: "rgba(245,200,66,0.07)"  },
  POSITIVE: { color: "#22e5a0", icon: "★", bg: "rgba(34,229,160,0.07)"  },
  HEDGE:    { color: "#8090a8", icon: "~", bg: "rgba(128,144,168,0.05)" },
  BEARISH:  { color: "#ff3d5a", icon: "▼", bg: "rgba(255,61,90,0.07)"   },
  BULLISH:  { color: "#22e5a0", icon: "▲", bg: "rgba(34,229,160,0.07)"  },
};

const DEMO_SEGMENTS = [
  { speaker: "CEO", text: "Good morning everyone. I am proud to report record revenue of 4.38 billion this quarter, significantly exceeding our guidance range and beating street expectations of 4.1 billion. Our AI-powered product suite drove 34 percent growth in enterprise bookings, well above the 22 percent the street had modeled." },
  { speaker: "CFO", text: "EPS came in at 2.67, beating consensus of 2.45. However gross margins contracted 80 basis points to 67.2 percent driven by elevated cloud infrastructure costs and data center capex. Free cash flow was 820 million, slightly below our internal target. Customer churn in enterprise ticked up to 6.2 percent versus 5.8 last year." },
...
function Gauge({ score }) {
  ...
  return (
    <svg viewBox="0 0 200 110" width="100%" style={{ maxWidth: 200, display: "block", margin: "0 auto" }}>
      ...
      {norm > 0.01 && <path d={`M${sx},${sy} A${r},${r},0,${norm > 0.5 ? 1 : 0},1,${fx},${fy}`} fill="none" stroke={color} strokeWidth="6" strokeLinecap="round" filter="url(#glow)"/>}
      <line x1={cx} y1={cy} x2={nx} y2={ny} stroke={color} strokeWidth="2.5" strokeLinecap="round" filter="url(#glow)"/>
...
function FlagCard({ flag, isNew }) {
  const cfg = SIGNAL_CFG[flag.type] || SIGNAL_CFG.HEDGE;
  const impC = { HIGH: "#ff3d5a", MEDIUM: "#f5c842", LOW: "#4a5a6a" };
  return (
    <div style={{ background: cfg.bg, border: `1px solid ${cfg.color}18`, borderLeft: `3px solid ${cfg.color}`, borderRadius: 8, padding: "10px 13px", marginBottom: 8, animation: isNew ? "slideUp .25s ease both" : "none" }}>
      <div style={{ display: "flex", gap: 5, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
        ...
        {flag.impact && <span style={{ fontSize: 9, color: impC[flag.impact] || "#4a5a6a", fontFamily: "monospace", marginLeft: "auto" }}>{flag.impact}</span>}
      </div>
...
export default function App() {
  ...
  useEffect(() => { checkHealth().then(setAiMode); }, []);

  const push = seg => { txRef.current = [...txRef.current, seg]; setTranscript([...txRef.current]); };

  const launch = async () => {
    setPhase("loading");
    setLoadMsg(`Fetching analyst context for ${ticker}…`);
    const { data, source } = await fetchMarket(ticker);
    mktRef.current = data;
    setMarket(data);
  ...
  const process = useCallback(async seg => {
    ...
    const { analysis: result, source } = await analyzeSegment(seg, ctx, mktRef.current, prior);
    setAnalysisSource(source);
    setAnalysis(result);
    setUpdates(u => u + 1);
    scoreRef.current = [...scoreRef.current.slice(-49), result.sentimentScore];
    setScores([...scoreRef.current]);
    if (result.flags?.length) {
      const tagged = result.flags.map((f, i) => ({ ...f, _id: `${Date.now()}-${i}` }));
  ...
  const addSegment = useCallback(async (text, spk) => {
    if (!text.trim()) return;
  ...
  const startMic = () => {
    ...
    timerRef.current = setInterval(async () => { const c = pending.current.trim(); if (c.length > 30) { pending.current = ""; await addSegment(c); } }, 12000);
  };

  const stopMic = () => {
    recRef.current?.stop(); clearInterval(timerRef.current); setListening(false);
    const c = pending.current.trim(); if (c.length > 5) { pending.current = ""; addSegment(c); }
  };

  const runDemo = async () => {
    if (busy) return;
    txRef.current = []; scoreRef.current = [];
    setTranscript([]); setAllFlags([]); setScores([]); setAnalysis(null); setUpdates(0);
  ...
  const submitPaste = async () => {
    ...
    if (paras.length <= 1) { await addSegment(paste, speaker); return; }
    for (const para of paras) {
      const m = para.match(/^\[(CEO|CFO|ANALYST)\]:\s*/i);
      await addSegment(m ? para.replace(m[0], "").trim() : para, m ? m[1].toUpperCase() : speaker);
  ...
  const reset = () => {
    ...
    setScores([]); setTranscript([]); setUpdates(0);
  };
  ...
  const biasCols = { STRONG_BUY: "#22e5a0", BUY: "#60cc99", HOLD: "#f5c842", SELL: "#ff8060", STRONG_SELL: "#ff3d5a" };
  const vsC = v => ({ BEAT:"#22e5a0", MISS:"#ff3d5a", INLINE:"#f5c842", UNKNOWN:"#2a3a48" })[v]||"#2a3a48";
  const vsI = v => ({ BEAT:"▲", MISS:"▼", INLINE:"≈", UNKNOWN:"—" })[v]||"—";

  const css = `
    @import url('https://fonts.googleapis.com/css2?family=Syne:wght@600;700;800&family=JetBrains+Mono:ital,wght@0,400;0,500;0,600;1,400&display=swap');
    *{box-sizing:border-box;margin:0;padding:0}
    @keyframes slideUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
    @keyframes pulse{0%,100%{opacity:1}50%{opacity:.2}}
    @keyframes glow{0%,100%{box-shadow:0 0 0 transparent}50%{box-shadow:0 0 28px rgba(34,229,160,0.1)}}
    @keyframes scanline{0%{transform:translateX(-100%)}100%{transform:translateX(100vw)}}
    @keyframes spin{to{transform:rotate(360deg)}}
    @keyframes fadeIn{from{opacity:0}to{opacity:1}}
    ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-track{background:#060d18}::-webkit-scrollbar-thumb{background:#1a2535}
    ...
    <div style={{ minHeight:"100vh", background:"#040b16", display:"flex", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ width:"100%", maxWidth:440, padding:40, animation:"fadeIn .4s ease" }}>
        <div style={{ textAlign:"center", marginBottom:40 }}>
          <div style={{ fontSize:44, color:"#22e5a0", marginBottom:12, filter:"drop-shadow(0 0 20px #22e5a050)" }}>◈</div>
          ...
          <p style={{ fontSize:12, color:"#2a4050", lineHeight:1.8, fontFamily:"'JetBrains Mono',monospace" }}>
            AI sentiment analysis for earnings calls.<br/>
            Mic · file · paste — vs analyst consensus.
          </p>
        </div>
        ...
        {phase === "loading" ? (
          <div style={{ textAlign:"center", padding:30 }}>
            ...
            <div style={{ fontSize:11, color:"#22e5a0", fontFamily:"'JetBrains Mono',monospace", letterSpacing:1 }}>{loadMsg}</div>
          </div>
        ) : (
          <>
            <div className="card" style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:9 }}>
                <label style={{ fontSize:9, color:"#1a3020", letterSpacing:3, textTransform:"uppercase" }}>Ticker Symbol</label>
                <span style={{ fontSize:8, padding:"2px 8px", borderRadius:4, background:aiMode?"rgba(34,229,160,0.07)":"rgba(245,200,66,0.07)", color:aiMode?"#22e5a0":"#f5c842", fontFamily:"'JetBrains Mono',monospace" }}>
                  {aiMode ? "● AI CONNECTED" : "● LOCAL MODE"}
                </span>
              </div>
              <input value={ticker} onChange={e => setTicker(e.target.value.toUpperCase())}
                placeholder="NVDA · MSFT · AAPL · AMZN"
              ...
              <div style={{ fontSize:9, color:"#111d2e", marginTop:8 }}>Presets: NVDA · MSFT · AAPL · AMZN</div>
            </div>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1360, margin:"0 auto", padding:"13px 15px" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:13, flexWrap:"wrap", gap:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <span style={{ fontSize:18, color:"#22e5a0", filter:"drop-shadow(0 0 8px #22e5a060)" }}>◈</span>
            ...
            {analysisSource && <span style={{ fontSize:8, padding:"2px 8px", borderRadius:4, background:analysisSource==="ai"?"rgba(34,229,160,0.07)":"rgba(245,200,66,0.07)", color:analysisSource==="ai"?"#22e5a0":"#f5c842" }}>{analysisSource==="ai"?"● CLAUDE AI":"● LOCAL NLP"}</span>}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ display:"flex", alignItems:"center", gap:5, padding:"3px 10px", background:"#070e1c", border:`1px solid ${listening?"#22e5a020":busy?"#5070e820":"#111d2e"}`, borderRadius:5 }}>
              ...
              <span style={{ fontSize:8, color:"#3a4a58", letterSpacing:1 }}>{listening?"LIVE MIC":busy?"ANALYZING…":updates>0?"STANDBY":"READY"}</span>
            </div>
            {updates > 0 && <span style={{ fontSize:8, color:"#1a2830" }}>#{updates}</span>}
        ...
        <div className="card" style={{ marginBottom:11 }}>
          <div style={{ display:"flex", gap:5, marginBottom:12, alignItems:"center", flexWrap:"wrap" }}>
            {[["mic","🎤 Mic"],["file","📁 File"],["paste","📋 Paste"]].map(([m,l]) => (
          ...
          {mode==="mic" && (
            <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
              <button onClick={listening?stopMic:startMic} style={{ padding:"7px 20px", borderRadius:7, border:"none", fontSize:9, letterSpacing:1, background:listening?"linear-gradient(135deg,#ff3d5a,#900020)":"linear-gradient(135deg,#1a6040,#22e5a0)", color:listening?"#fff":"#040b16", fontWeight:700 }}>{listening?"⏹ STOP":"🎤 START"}</button>
              <button onClick={runDemo} disabled={busy} style={{ padding:"7px 16px", borderRadius:7, border:"1px solid #1e3060", background:"transparent", color:"#4060c0", fontSize:9, opacity:busy?0.4:1 }}>▶ DEMO</button>
              <span style={{ fontSize:9, color:"#1a2530" }}>Chrome/Edge · analyzes every 12s</span>
            </div>
          )}
          {mode==="file" && (
            <div style={{ display:"flex", gap:10, alignItems:"center" }}>
              <button onClick={() => fileRef.current?.click()} style={{ padding:"7px 16px", borderRadius:7, border:"1px solid #111d2e", background:"#040b16", color:"#8090a0", fontSize:9 }}>CHOOSE FILE</button>
          ...
          {mode==="paste" && (
            <div style={{ display:"flex", gap:8 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                <button onClick={submitPaste} disabled={busy||!paste.trim()} style={{ padding:"9px 14px", borderRadius:7, border:"none", background:"linear-gradient(135deg,#101a40,#0c1430)", color:"#4060c0", fontSize:9, opacity:(busy||!paste.trim())?0.4:1, fontWeight:600 }}>ANALYZE →</button>
                <button onClick={runDemo} disabled={busy} style={{ padding:"9px 14px", borderRadius:7, border:"1px solid #1e3060", background:"transparent", color:"#4060c0", fontSize:9, opacity:busy?0.4:1 }}>▶ DEMO</button>
              </div>
            </div>
          )}
        </div>
        ...
        {tab==="dash" && (
          <div style={{ display:"grid", gridTemplateColumns:"210px 1fr 210px", gap:11 }}>
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              ...
              {analysis?.priceActionBias && <div className="card">
                <div style={{ fontSize:8, color:"#1a2a38", letterSpacing:1, marginBottom:6 }}>PRICE ACTION BIAS</div>
                <div style={{ fontSize:13, fontWeight:700, color:biasCols[analysis.priceActionBias]||"#f5c842", letterSpacing:1 }}>{analysis.priceActionBias.replace("_"," ")}</div>
                <div style={{ fontSize:9, color:"#3a4a58", marginTop:4 }}>Est. AH: {analysis.estimatedMove}</div>
              </div>}
              {analysis?.vsConsensus && <div className="card">
              ...
              {scores.length>1 && <div className="card">
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              {analysis ? (<>
                ...
                <div className="card" style={{ flex:1 }}>
                  ...
                  {analysis.keyThemes?.length>0 && <div style={{ marginTop:14 }}>
                  ...
                  {analysis.redFlags?.length>0 && <div style={{ marginTop:14 }}>
                    <div style={{ fontSize:8, color:"#1a2a38", letterSpacing:1, marginBottom:8 }}>RED FLAGS</div>
                    {analysis.redFlags.map((f,i)=><div key={i} style={{ fontSize:10, color:"#cc5060", marginBottom:4, paddingLeft:9, borderLeft:"2px solid #ff3d5a20" }}>⚑ {f}</div>)}
                  </div>}
                </div>
                {allFlags.slice(0,2).length>0 && <div className="card">
                  ...
                  {allFlags.length>2 && <button onClick={()=>setTab("flags")} style={{ fontSize:9, color:"#22e5a0", background:"transparent", border:"none", padding:0, marginTop:4 }}>+ {allFlags.length-2} more →</button>}
                </div>}
              </>) : (
                <div className="card" style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:60, color:"#111d2e", minHeight:280, border:"1px dashed #111d2e" }}>
                  ...
                  <div style={{ fontSize:10, letterSpacing:2, textAlign:"center", lineHeight:2.5 }}>PASTE TRANSCRIPT<br/>START MIC<br/>OR HIT ▶ DEMO</div>
                </div>
              )}
            </div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              {market && (<>
                <div className="card">
                  ...
                  <div style={{ marginBottom:10 }}><div style={{ fontSize:8, color:"#3a4a58", marginBottom:3 }}>EPS</div><div style={{ fontSize:20, color:"#b0cce0", fontWeight:600 }}>${market.epsConsensus}</div></div>
                  <div><div style={{ fontSize:8, color:"#3a4a58", marginBottom:3 }}>Revenue</div><div style={{ fontSize:20, color:"#b0cce0", fontWeight:600 }}>${market.revenueConsensus}{market.revenueUnit}</div></div>
                </div>
                <div className="card">
                  <div style={{ fontSize:8, color:"#1a2a38", letterSpacing:1, marginBottom:12 }}>RATINGS</div>
                ...
                <div className="card">
                  ...
                  {market.keyWatchPoints?.map((w,i)=><div key={i} style={{ fontSize:9, color:"#2a3848", marginBottom:5, paddingLeft:8, borderLeft:"2px solid #111d2e", lineHeight:1.5 }}>⚑ {w}</div>)}
                </div>
                <div className="card">
        ...
        {tab==="flags" && <div className="card">
          <div style={{ fontSize:8, color:"#1a2a38", letterSpacing:1, marginBottom:14 }}>ALL SIGNALS — {allFlags.length}</div>
          {allFlags.length===0 ? <div style={{ textAlign:"center", padding:55, color:"#111d2e", fontSize:10 }}>No signals yet</div>
            : <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:8 }}>{allFlags.map((f,i)=><FlagCard key={f._id||i} flag={f} isNew={freshIds.has(f._id)}/>)}</div>}
        </div>}

        {tab==="tx" && <div className="card">
          ...
          <div style={{ maxHeight:500, overflowY:"auto" }}>
            ...
            {listening && <span style={{ fontSize:11, color:"#22e5a0", animation:"pulse 1s ease infinite" }}>▌</span>}
          </div>
        </div>}

        {tab==="mkt" && market && <div className="card">
          <div style={{ fontSize:8, color:"#1a2a38", letterSpacing:1, marginBottom:14 }}>MARKET CONTEXT — {market.ticker}</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:11, marginBottom:18 }}>
            {[["EPS Consensus",`$${market.epsConsensus}`],["Revenue",`$${market.revenueConsensus}${market.revenueUnit}`],["Price Target",`$${market.priceTargetAvg}`],["Short Interest",market.shortInterest],["Sentiment",market.streetSentiment]].map(([l,v])=>(
